...
LeptonProducer::LeptonProducer(const edm::ParameterSet& iConfig)
{
  ...
  produces<std::vector<float> >("ElectronIsoValue");
  produces<std::vector<float> >("MuonIsoValue");
  produces<int>("ElecHandleSize");
  produces<int>("MuonHandleSize");
...
void LeptonProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  std::auto_ptr<std::vector<float> > electronIsoValue(new std::vector<float>());
  std::auto_ptr<std::vector<float> > muonIsoValue(new std::vector<float>());
  std::auto_ptr<int> elecHandleSize(new int());
  std::auto_ptr<int> muonHandleSize(new int());
  ...
  std::vector<float> muonIsoValue_, electronIsoValue_;
  int elecHandleSize_=0;
  int muonHandleSize_=0;

  if(muonHandle.isValid())
    muonHandleSize_=muonHandle->size();
    {
      for(unsigned int m=0; m<muonHandle->size(); ++m)
	{
      bool verbose = false;
      bool softtest=false;
      if(muonHandle->at(m).pt()<5.0) softtest=true;

	  if(muonHandle->at(m).pt()<minMuPt_ || fabs(muonHandle->at(m).eta())>maxMuEta_) continue;
      if(softtest && verbose) std::cout << "Step 1: muon pt < 5.0 && eta<maxEta" << std::endl; 

	  if(MuonID(muonHandle->at(m),vtx_h->at(0)))
	    {
          if(softtest && verbose) std::cout << "Step 2: Muon ID passed!" << std::endl;
          ...
          if(verbose && softtest) std::cout<<"IsoCriterium<IsoValue: "<<dBIsoMu<<"<"<<muIsoValue_<<std::endl;
          muonIsoValue_.push_back(dBIsoMu);
	      if(dBIsoMu<muIsoValue_)
		{
		  isoMuons_.push_back(muonHandle->at(m));
          if(softtest && verbose)std::cout<<"Step 3: Pass isolation criteria"<<std::endl;
		  Leptons++;
  ...
  if(eleHandle.isValid())
    {
      elecHandleSize_=eleHandle->size();
	  ...
	  if(ElectronID(aEle, vtx, VETO)) // check VETO id
	    {
          ...
          electronIsoValue_.push_back(miniIso);
	      ...
	      //std::cout << "Electron isolation value : " << elecIsoValue_ << std::endl;
  ...
  *electronIsoValue = electronIsoValue_;
  *muonIsoValue = muonIsoValue_;
  *elecHandleSize = elecHandleSize_;
  *muonHandleSize = muonHandleSize_;
  ...
  iEvent.put(elecHandleSize, "ElecHandleSize");
  iEvent.put(muonHandleSize, "MuonHandleSize");
  ...
  iEvent.put(electronIsoValue, "ElectronIsoValue");
  iEvent.put(muonIsoValue, "MuonIsoValue");
}
